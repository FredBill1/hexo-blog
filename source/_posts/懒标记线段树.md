...
S op(S a, S b) {
    ...
    if (a.max == b.max) {
        max = a.max;
        max_cnt = a.max_cnt + b.max_cnt;
        max_2 = std::max(a.max_2, b.max_2);
    } else if (a.max > b.max) {
        max = a.max;
        max_cnt = a.max_cnt;
        max_2 = std::max(a.max_2, b.max);
    } else {
        max = b.max;
        max_cnt = b.max_cnt;
        max_2 = std::max(a.max, b.max_2);
    }
    int min, min_2;
    int min_cnt;
    if (a.min == b.min) {
        min = a.min;
        min_cnt = a.min_cnt + b.min_cnt;
        min_2 = std::min(a.min_2, b.min_2);
    } else if (a.min < b.min) {
        min = a.min;
        min_cnt = a.min_cnt;
        min_2 = std::min(a.min_2, b.min);
    } else {
        min = b.min;
        min_cnt = b.min_cnt;
        min_2 = std::min(a.min, b.min_2);
    }
    return S{
        .sum = sum,
        .len = len,
...
optional<S> mapping(F f, S x) {
    if (f.add) {
        ...
        x.max += f.add;
        x.min += f.add;
        if (x.max_2 != NEG_INF) x.max_2 += f.add;
        if (x.min_2 != INF) x.min_2 += f.add;
    }
    // 无法确定需要更新的数的数目，需要向下暴力递归搜索，再向上更新回来
    if (f.upper_bound <= x.max_2 || f.lower_bound >= x.min_2) return nullopt;
    if (f.upper_bound < x.max) {
        x.sum -= ll(x.max - f.upper_bound) * x.max_cnt;
        if (x.min == x.max) x.min = f.upper_bound;
        if (x.min_2 == x.max) x.min_2 = f.upper_bound;
        x.max = f.upper_bound;
    }
    if (f.lower_bound > x.min) {
        x.sum += ll(f.lower_bound - x.min) * x.min_cnt;
        if (x.max == x.min) x.max = f.lower_bound;
        if (x.max_2 == x.min) x.max_2 = f.lower_bound;
        x.min = f.lower_bound;
    }
    return x;
}
...
F composition(F f, F g) {
    ...
    return F{
        .add = f.add + g.add,
        .upper_bound = std::min(f.upper_bound, std::max(f.lower_bound, g.upper_bound)),
        .lower_bound = std::max(f.lower_bound, std::min(f.upper_bound, g.lower_bound)),
    };
}
// id: 函数的单位元，即mapping(id, x) = id(x) = x
F id() { return F{}; }
