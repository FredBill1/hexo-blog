...
// f(i, j, k): j == (i + 1) % chull.size(), k is the farthest point from the line (points[i], points[j])
...
void rotating_calipers(const std::vector<Vec2<T>>& points, const std::vector<int>& chull, F&& f,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (;;) {
            int k_ = k + 1 == N ? 0 : k + 1;
            auto old_area = (points[chull[k]] - points[chull[j]]).cross(points[chull[i]] - points[chull[j]]);
            auto new_area = (points[chull[k_]] - points[chull[j]]).cross(points[chull[i]] - points[chull[j]]);
            if (stop_at_first ? new_area <= old_area : new_area < old_area) break;
            k = k_;
        }
