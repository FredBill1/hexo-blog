...
class dynamic_segtree {
    ...
    int _low, _high;
    Node* _root = nullptr;
    Node* _build(const std::vector<S>& v, int s, int t) {
        Node* root = new Node();
        if (s == t) {
            root->val = v[s];
            return root;
    ...
    void _set(Node*& root, int p, S x, int s, int t) {
    ...
    S _get(const Node* root, int p, int s, int t) const {
    ...
    S _prod(const Node* root, int l, int r, int s, int t) const {
        if (root == nullptr) return e();
    ...
    void _merge(Node*& root, Node*& other, int s, int t) {
    ...
    void _split(Node*& old_root, Node*& new_root, int l, int r, int s, int t) {
    ...
    template <class F>
    int _max_right(const Node* root, int l, F f, S& sm, int s, int t) const {
    ...
    template <class F>
    int _min_left(const Node* root, int r, F f, S& sm, int s, int t) const {
