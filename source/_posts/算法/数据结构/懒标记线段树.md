...
模板的接口文档在[这里](https://atcoder.github.io/ac-library/production/document_en/lazysegtree.html)，使用方法参考例题代码中的注释。这个模板中的一些命名与其它常见的模板有一定区别：

- `S`相当于保存节点信息的结构体
- `F`相当于每个节点对应的懒标记(`tag`)
- `op`相当于`pushup`，即用两个子节点的信息(结构体`S`)更新当前节点的信息
- `composition`相当于`pushdown`时利用当前节点的懒标记更新子节点懒标记的操作
- `mapping`相当于`update`时基于懒标记对结点的操作
- `prod`相当于`query`
- `apply`相当于`update`
