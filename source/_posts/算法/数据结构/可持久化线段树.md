...
class persistent_segtree {
    ...
    explicit persistent_segtree(int n, const Node* root) : _n(n), _root(root) {}
   ...
   public:
    ...
    S get(int p) const {
        assert(0 <= p && p < _n);
        return _get(_root, p, 0, _n - 1);
    }
    S prod(int l, int r) const {
        assert(0 <= l && l <= r && r <= _n);
        if (l == r) return e();
        return _prod(_root, l, r - 1, 0, _n - 1);
    }
    S all_prod() const { return _root->val; }
};
```
